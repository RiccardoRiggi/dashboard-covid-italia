...
router.get('/storico-rapporto-tamponi-positivi', (req, res) => {
    let sql = 'SELECT sto.tamponi as valoreTotale, ROUND(nuovi_positivi * 100 / IFNULL(sto.tamponi-(SELECT sn.tamponi FROM storico_nazionale sn WHERE sn.data = subdate(sto.data, 1)),0),1) as differenza, sto.data as giorno FROM storico_nazionale sto ORDER BY sto.data';
    let query = con.query(sql, (err, results) => {
        try {
            if (err) throw err;
            res.setHeader('Content-Type', 'application/json');
