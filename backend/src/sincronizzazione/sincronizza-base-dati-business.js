...
module.exports = {
  sincronizzaDatiNazionali: async function () {
    ...
    for (var c = 0; c < elencoDatiNazionali.length; c++) {
      ...
      try {
        var sql = "INSERT INTO `storico_nazionale` (`data`, `stato`, `ricoverati_con_sintomi`, `terapia_intensiva`,`totale_ospedalizzati`,`isolamento_domiciliare`,`totale_positivi`,`variazione_totale_positivi`,`nuovi_positivi`,`dimessi_guariti`,`deceduti`,`casi_da_sospetto_diagnostico`,`casi_da_screening`,`totale_casi`,`tamponi`,`casi_testati` , `note`) VALUES ( \"" + tmp.data + "\" , \"" + tmp.stato + "\" ," + tmp.ricoverati_con_sintomi + " ," + tmp.terapia_intensiva + " ," + tmp.totale_ospedalizzati + " ," + tmp.isolamento_domiciliare + " ," + tmp.totale_positivi + " ," + tmp.variazione_totale_positivi + " ," + tmp.nuovi_positivi + " ," + tmp.dimessi_guariti + " ," + tmp.deceduti + " ," + tmp.casi_da_sospetto_diagnostico + " ," + tmp.casi_da_screening + " ," + tmp.totale_casi + " ," + tmp.tamponi + " ," + tmp.casi_testati + ",\"" + tmp.note + "\" ) ";
  ...
  sincronizzaDatiRegionali: async function () {
    ...
    for (var c = 0; c < elencoDatiRegionali.length; c++) {
      ...
      tmp.data = year + "-" + month + "-" + date + " " + hours + ":" + minutes + ":" + seconds;
      try {
        var sql = "INSERT INTO `storico_regioni` (`data`, `stato`, `codice_regione`, `denominazione_regione`, `lat`, `long`, `ricoverati_con_sintomi`, `terapia_intensiva`,`totale_ospedalizzati`,`isolamento_domiciliare`,`totale_positivi`,`variazione_totale_positivi`,`nuovi_positivi`,`dimessi_guariti`,`deceduti`,`casi_da_sospetto_diagnostico`,`casi_da_screening`,`totale_casi`,`tamponi`,`casi_testati` , `note`) VALUES ( \"" + tmp.data + "\" , \"" + tmp.stato + "\" , " + tmp.codice_regione + " ,\"" + tmp.denominazione_regione + "\"  ," + tmp.lat + " ," + tmp.long + " ," + tmp.ricoverati_con_sintomi + " ," + tmp.terapia_intensiva + " ," + tmp.totale_ospedalizzati + " ," + tmp.isolamento_domiciliare + " ," + tmp.totale_positivi + " ," + tmp.variazione_totale_positivi + " ," + tmp.nuovi_positivi + " ," + tmp.dimessi_guariti + " ," + tmp.deceduti + " ," + tmp.casi_da_sospetto_diagnostico + " ," + tmp.casi_da_screening + " ," + tmp.totale_casi + " ," + tmp.tamponi + " ," + tmp.casi_testati + ",\"" + tmp.note + "\" ) ";
        con.query(sql, function (err, result) {
          if (!err)
            contatore++;
          console.log("Record inseriti: " + contatore + " - Tentativi: " + c);
