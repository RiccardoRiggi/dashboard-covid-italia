...
export class GiornoSpecificoProvinciaComponent implements OnInit {
  ...
  constructor(httpDue: HttpClient, route: ActivatedRoute) { 
    ...
    let obs = httpDue.get(environment.url_base+'province/'+this.codiceRegione+'/'+this.codiceProvincia+'/dettaglioStorico', { responseType: 'text' });
    obs.subscribe((ResponseDue) => this.mettiJSONDue((ResponseDue)));
    obs = httpDue.get(environment.url_base+'menu/elencoProvince/'+this.codiceRegione+'/'+this.codiceProvincia, { responseType: 'text' });
